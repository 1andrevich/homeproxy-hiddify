...
Кнопка **Импорт share-ссылок** принимает по одной ссылке на строку. Поддерживаемые схемы:
...
Кнопка **Импорт .conf** читает файл конфигурации **WireGuard** или **AmneziaWG** `.conf` и создаёт из него узел — удобно для готовых конфигов WARP/AmneziaWG. (Узлы AmneziaWG требуют ядро **sing-box-extended** — см. [Поддерживаемые протоколы](Supported-Protocols-ru).)
