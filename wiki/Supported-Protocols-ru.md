...
Разбивает TLS ClientHello на несколько TCP-пакетов. Многие системы DPI (глубокая инспекция пакетов) анализируют только первый пакет для идентификации трафика — фрагментация лишает их этой возможности, так как ни один пакет не содержит достаточно данных для срабатывания блокировки. Можно включить для протоколов, использующих TLS (VLESS, VMess, Trojan и др.).
