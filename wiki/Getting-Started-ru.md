...
Приложение LuCI — это только интерфейс; проксированием занимается отдельный бинарник **ядра**. Откройте **Статус → Управление ядром** и установите одно из них:
...
На странице **Клиент** задайте **Основной узел** = добавленный узел — или выберите **URLTest**, чтобы Re:HomeProxy автоматически выбирал самый быстрый доступный узел и переключался при сбоях.

---

## Шаг 4 — Выберите режим маршрутизации

На вкладке **Клиент → Маршрутизация** выберите **Режим маршрутизации**. Значение по умолчанию зависит от языка LuCI:

- **Россия (прокси запрещён)** — `proxy_banned_ru`, по умолчанию для русских установок: всё идёт **напрямую**, кроме адресов, добавленных в **RU Proxy Rules** (заблокированные/троттлящиеся сайты) — они идут через прокси.
- **Глобально** — весь трафик через прокси.
- **GFWList / Обход материкового Китая / Только материковый Китай** — пресеты для Китая.
- **Пользовательская маршрутизация / Пользовательский JSON** — полный ручной контроль.

Для России оставьте **Россия (прокси запрещён)** и откройте вкладку **RU Proxy Rules**, чтобы включить нужные списки (Russia Inside, Re:Filter, YouTube, Discord…). Полный справочник: **[Маршрутизация и контроль доступа](Routing-and-Access-Control-ru)**.

---

## Шаг 5 — Проверьте базовые настройки транспорта

На той же вкладке **Маршрутизация** значения по умолчанию разумны:
...
- **Связь** — проверяет доступность тестовых сайтов и показывает ваш **Прямой IP** против **IP через прокси** (при проксировании они должны различаться).
...
## По желанию — снять троттлинг без VPN-узла

Если сайт (например, YouTube, Discord) просто *троттлится*, а не заблокирован по IP, его можно отдать встроенному обходу DPI вместо расхода трафика VPN:

- **[ByeDPI](ByeDPI-ru)** — рассинхронизация на уровне SOCKS.
- **[Zapret](Zapret-ru)** — рассинхронизация на уровне пакетов (nfqws2).

Оба выбираются для конкретного правила RU Proxy Rules (например, направить через ByeDPI/Zapret только YouTube, а остальное — через VPN).
