...
## Нет интернета при включённом HomeProxy (конфликт с mwan3)

**Симптом:** Ping и DNS работают, но HTTP/HTTPS-трафик не проходит при включённом HomeProxy. Остановка mwan3 восстанавливает связь.

**Причина:** mwan3 использует fwmark и policy routing для распределения трафика между WAN-интерфейсами. Когда hiddify-core устанавливает исходящие соединения, mwan3 может перехватить их и направить не туда.

**Решение:** Сообщить ОС пропускать логику маршрутизации mwan3 для трафика, уже обработанного HomeProxy (помечен `0x64`).
